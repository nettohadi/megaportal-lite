...
export type UseDataTableAsyncSearchOptions<
	TData,
...
export function useDataTableAsyncSearch<
	TData,
	TVariables extends BaseVariables
>(options: UseDataTableAsyncSearchOptions<TData, TVariables>) {
	const {
		data,
		fetch,
		filterLabelKey,
	...
	const searchData = useCallback<AsyncSelectSearchData<TData>>(
		(input) => {
			if (!searchCriteria) {
				throw new Error('No searchCriteria provided.');
			}

			return fetch({ input, searchCriteria } as TVariables)
				.then((result) => result ?? [])
				.catch((err) => {
